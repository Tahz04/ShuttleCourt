...
// THÊM DÒNG IMPORT NÀY CHO MÀN HÌNH CHỦ SÂN (Đảm bảo bạn đã tạo file này)
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                ...
                // --- LOGIC GỌI API NÂNG CẤP CHỦ SÂN ---
                InkWell(
                  onTap: () {
                    final authService = Provider.of<AuthService>(context, listen: false);
                    ...
                    if (authService.user?.role == 'owner') {
                      Navigator.push(context, MaterialPageRoute(builder: (context) => const OwnerDashboardScreen()));
                    } else {
                      showDialog(
                        context: context,
                        builder: (context) => AlertDialog(
                          title: const Text('Đăng ký Đối tác'),
                          content: const Text('Tài khoản của bạn hiện là Người dùng bình thường. Bạn có muốn nâng cấp lên Đối tác/Chủ sân để quản lý sân không?'),
                          actions: [
                            TextButton(
                            ...
                            TextButton(
                              onPressed: () async {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Đang xử lý nâng cấp...')),
                                );

                                // Gọi API nâng cấp từ AuthService
                                ...
                                if (success) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Chúc mừng! Bạn đã trở thành Đối tác/Chủ sân.', style: TextStyle(color: Colors.white)),
                                      backgroundColor: Colors.green,
                                    ),
                                  );
                                  // Chuyển thẳng vào màn hình Dashboard
                                  Navigator.push(context, MaterialPageRoute(builder: (context) => const OwnerDashboardScreen()));
                                } else {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(authService.errorMessage ?? 'Nâng cấp thất bại, vui lòng thử lại.', style: const TextStyle(color: Colors.white)),
                  ...
                  child: _buildFeatureButton(Icons.store, 'Dành cho Chủ sân', Colors.purple),
                ),
                // ----------------------------------------------------
